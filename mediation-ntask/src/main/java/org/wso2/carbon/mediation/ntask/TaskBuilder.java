...
final class TaskBuilder {
	...
	public static TaskInfo buildTaskInfo(TaskDescription taskDescription,
	...
	public static TaskInfo buildTaskInfo(TaskDescription description,
		...
		if (cron == null && !system && description.getInterval() < 1000) {
			throw new Exception(
					"Task interval cannot be less than 1 second for user tasks");
		}
		triggerInfo.setIntervalMillis((int) description.getInterval());
		triggerInfo.setRepeatCount(description.getCount() > 0 ? description
				.getCount() - 1 : description.getCount());
		...
		String nameGroup = description.getName() + "::"
				+ description.getTaskGroup();
		...
		return new TaskInfo(description.getName(),
				NTaskAdapter.class.getName(), props,
				RoundRobinTaskLocationResolver.class.getName(), triggerInfo);
	}
