 ...
 * letter channel EIP It will Time to time Redeliver the Messages to a given
 * target.
 */
public class ScheduledMessageForwardingProcessor extends
	...
	public void init(SynapseEnvironment se) {
		parameters.put(
				ForwardingProcessorConstants.THROTTLE,
				String.valueOf(isThrottling(interval)
						|| isThrottling(cronExpression)));
		if (isThrottling(cronExpression)) {
			parameters.put(ForwardingProcessorConstants.THROTTLE_INTERVAL,
					String.valueOf(interval));
		}

		if (nonRetryStatusCodes != null) {
			parameters.put(ForwardingProcessorConstants.NON_RETRY_STATUS_CODES,
					nonRetryStatusCodes);
		}

		// Setting the end-point here.
		parameters.put(ForwardingProcessorConstants.TARGET_ENDPOINT,
				targetEndpoint);
		super.init(se);
		...
		org.apache.synapse.commons.jmx.MBeanRegistrar.getInstance()
				.registerMBean(view, "Message Forwarding Processor view",
						getName());
	}
	...
	protected Task getTask() {
		return new ForwardingService(this, sender, synapseEnvironment);
	}
}
